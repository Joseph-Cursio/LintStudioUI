...
@MainActor
@Suite("GitServiceActor Tests")
struct GitServiceTests {

    // MARK: - Helpers

    private let fixtureName = "sample.txt"
    ...
    private func createTempGitRepo() throws -> URL {
        ...
        let configContent = "alpha\nbeta\n"
        let configPath = tempDir.appendingPathComponent(fixtureName)
    ...
    func testShowFile() async throws {
        ...
        let service = GitServiceActor()
        let content = try await service.showFile(at: repoDir, branch: currentBranch, filePath: fixtureName)
    ...
    func testShowFileFromDifferentBranch() async throws {
        ...
        let newConfig = "gamma\ndelta\n"
        try newConfig.write(
            to: repoDir.appendingPathComponent(fixtureName),
        ...
        let service = GitServiceActor()
        let content = try await service.showFile(at: repoDir, branch: "other-config", filePath: fixtureName)
    ...
    func testDiffFile() async throws {
        ...
        let diff = try await service.diffFile(
            at: repoDir, fromRef: currentBranch, toRef: "diff-branch", filePath: fixtureName
        )
